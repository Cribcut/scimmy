...
const lib_resources = require('./resources.cjs');
...
class ErrorMessage extends Error {
    ...
    constructor(ex = {}) {
        ...
        if (schemas.includes(ErrorMessage.#id))
            throw new lib_types.Types.Error(status, scimType, detail);
...
class PatchOp {
    ...
    constructor(request) {
        ...
        if (this.#dispatched && (schemas.length !== 1 || !schemas.includes(PatchOp.#id)))
            throw new lib_types.Types.Error(400, "invalidSyntax", `PatchOp request body messages must exclusively specify schema as '${PatchOp.#id}'`);
        
        // Make sure request body contains valid operations to perform
        if (!Array.isArray(operations))
            throw new lib_types.Types.Error(400, "invalidValue", "PatchOp expects 'Operations' attribute of 'request' parameter to be an array");
        if (this.#dispatched && !operations.length)
            throw new lib_types.Types.Error(400, "invalidValue", "PatchOp request body must contain 'Operations' attribute with at least one operation");
        ...
        for (let operation of operations) {
            ...
            if (Object(operation) !== operation || Array.isArray(operation))
                throw new lib_types.Types.Error(400, "invalidValue", `PatchOp request body expected value type 'complex' for operation ${index} but found type '${Array.isArray(operation) ? "collection" : typeof operation}'`);
            // Make sure all operations have a valid action defined
            if (op === undefined)
                throw new lib_types.Types.Error(400, "invalidValue", `Missing required attribute 'op' from operation ${index} in PatchOp request body`);
            if (typeof op !== "string" || !validOps.includes(op.toLowerCase()))
                throw new lib_types.Types.Error(400, "invalidSyntax", `Invalid operation '${op}' for operation ${index} in PatchOp request body`);
            
            // Make sure value attribute is specified for "add" operations
            if ("add" === op.toLowerCase() && value === undefined)
                throw new lib_types.Types.Error(400, "invalidValue", `Missing required attribute 'value' for 'add' op of operation ${index} in PatchOp request body`);
            // Make sure path attribute is specified for "remove" operations
            if ("remove" === op.toLowerCase() && path === undefined)
                throw new lib_types.Types.Error(400, "noTarget", `Missing required attribute 'path' for 'remove' op of operation ${index} in PatchOp request body`);
            // Make sure path attribute is a string
            if (path !== undefined && typeof path !== "string")
                throw new lib_types.Types.Error(400, "invalidPath", `Invalid path '${path}' for operation ${index} in PatchOp request body`);
        }
    ...
    async apply(resource, finalise) {
        ...
        if ((resource === undefined) || !(resource instanceof lib_types.Types.Schema))
    ...
    #resolve(index, path, op) {
        ...
        try {
            ...
            throw new lib_types.Types.Error(400, "invalidPath", `Invalid path '${path}' for '${op}' op of operation ${index} in PatchOp request body`);
        }
            ...
            for (let target of targets.splice(0)) {
                if (target !== undefined) try {
                    if (filter !== undefined) {
                        ...
                        targets.push(...(new lib_types.Types.Filter(filter.substring(1, filter.length - 1), this.#schema?.definition).match(target[key])));
                    } else {
        ...
        if (targets.length === 0 && op !== "remove")
            throw new lib_types.Types.Error(400, "noTarget", `Filter '${path}' does not match any values for '${op}' op of operation ${index} in PatchOp request body`);
         ...
         */
        return {
            complex: (attribute instanceof lib_types.Types.SchemaDefinition ? true : attribute.type === "complex"),
    ...
    #add(index, path, value) {
        if (path === undefined) {
            ...
            if (typeof value !== "object" || Array.isArray(value))
                throw new lib_types.Types.Error(400, "invalidValue", `Attribute 'value' must be an object when 'path' is empty for 'add' op of operation ${index} in PatchOp request body`);
            ...
            for (let target of targets) {
                try {
                    ...
                    if (ex instanceof lib_types.Types.Error) {
                        ...
                        throw new lib_types.Types.Error(400, "invalidPath", `Invalid attribute path '${property}' in supplied value for 'add' op of operation ${index} in PatchOp request body`);
                    } else {
                        // Rethrow exceptions as SCIM errors
                        throw new lib_types.Types.Error(400, "invalidValue", ex.message + ` for 'add' op of operation ${index} in PatchOp request body`);
                    }
                }
            }
        }
                ...
                try {
                    ...
                    // Multivalued target, attempt removal of matching values from attribute
                        ...
                        const removals = (!complex || values.every(v => Object.isFrozen(v)) ? values : (
                            new lib_types.Types.Filter(values.map(f => Object.entries(f)
                    ...
                    if (ex instanceof lib_types.Types.Error) {
                        ...
                        // Handle errors caused by non-existent attributes in complex values
                        throw new lib_types.Types.Error(400, "invalidPath", `Invalid attribute path '${property}' in supplied value for 'remove' op of operation ${index} in PatchOp request body`);
                    } else {
                        // Rethrow exceptions as SCIM errors
                        throw new lib_types.Types.Error(400, "invalidValue", ex.message + ` for 'remove' op of operation ${index} in PatchOp request body`);
                    }
                }
            }
        } else {
...
class BulkRequest {
    ...
    constructor(request, maxOperations = 0) {
        ...
        if (schemas.length !== 1 || !schemas.includes(BulkRequest.#id))
            throw new lib_types.Types.Error(400, "invalidSyntax", `BulkRequest request body messages must exclusively specify schema as '${BulkRequest.#id}'`);
        // Make sure failOnErrors is a valid integer
        if (typeof failOnErrors !== "number" || !Number.isInteger(failOnErrors) || failOnErrors < 0)
            throw new lib_types.Types.Error(400, "invalidSyntax", "BulkRequest expected 'failOnErrors' attribute of 'request' parameter to be a positive integer");
        // Make sure maxOperations is a valid integer
        if (typeof maxOperations !== "number" || !Number.isInteger(maxOperations) || maxOperations < 0)
            throw new lib_types.Types.Error(400, "invalidSyntax", "BulkRequest expected 'maxOperations' parameter to be a positive integer");
        // Make sure request body contains valid operations to perform
        if (!Array.isArray(operations))
            throw new lib_types.Types.Error(400, "invalidValue", "BulkRequest expected 'Operations' attribute of 'request' parameter to be an array");
        if (!operations.length)
            throw new lib_types.Types.Error(400, "invalidValue", "BulkRequest request body must contain 'Operations' attribute with at least one operation");
        if (maxOperations > 0 && operations.length > maxOperations)
            throw new lib_types.Types.Error(413, null, `Number of operations in BulkRequest exceeds maxOperations limit (${maxOperations})`);
     ...
     */
    async apply(resourceTypes = Object.values(lib_resources.Resources.declared()), ctx) {
        ...
        else if (!resourceTypes.every(r => r.prototype instanceof lib_types.Types.Resource))
        ...
        for (let op of this.Operations) results.push((async () => {
            ...
            if (!method && method !== false)
                error = new ErrorMessage(new lib_types.Types.Error(400, "invalidSyntax", `Missing or empty 'method' string ${errorSuffix}`));
            // Make sure that value is a string
            else if (typeof method !== "string")
                error = new ErrorMessage(new lib_types.Types.Error(400, "invalidSyntax", `Expected 'method' to be a string ${errorSuffix}`));
            // Make sure that string is a valid method
            else if (!validMethods.includes(String(method).toUpperCase()))
                error = new ErrorMessage(new lib_types.Types.Error(400, "invalidValue", `Invalid 'method' value '${method}' ${errorSuffix}`));
            // Make sure path has a value
            else if (!path && path !== false)
                error = new ErrorMessage(new lib_types.Types.Error(400, "invalidSyntax", `Missing or empty 'path' string ${errorSuffix}`));
            // Make sure that path is a string
            else if (typeof path !== "string")
                error = new ErrorMessage(new lib_types.Types.Error(400, "invalidSyntax", `Expected 'path' to be a string ${errorSuffix}`));
            // Make sure that string points to a valid resource type
            else if (![...typeMap.keys()].includes(`/${endpoint}`))
                error = new ErrorMessage(new lib_types.Types.Error(400, "invalidValue", `Invalid 'path' value '${path}' ${errorSuffix}`));
            // Make sure there IS a bulkId if the method is POST
            else if (method.toUpperCase() === "POST" && !bulkId && bulkId !== false)
                error = new ErrorMessage(new lib_types.Types.Error(400, "invalidSyntax", `POST operation missing required 'bulkId' string ${errorSuffix}`));
            // Make sure there IS a bulkId if the method is POST
            else if (method.toUpperCase() === "POST" && typeof bulkId !== "string")
                error = new ErrorMessage(new lib_types.Types.Error(400, "invalidValue", `POST operation expected 'bulkId' to be a string ${errorSuffix}`));
            // Make sure there ISN'T a resource targeted if the method is POST
            else if (method.toUpperCase() === "POST" && !!id)
                error = new ErrorMessage(new lib_types.Types.Error(404, null, `POST operation must not target a specific resource ${errorSuffix}`));
            // Make sure there IS a resource targeted if the method isn't POST
            else if (method.toUpperCase() !== "POST" && !id)
                error = new ErrorMessage(new lib_types.Types.Error(404, null, `${method.toUpperCase()} operation must target a specific resource ${errorSuffix}`));
            // Make sure data is an object, if method isn't DELETE
            else if (method.toUpperCase() !== "DELETE" && (Object(data) !== data || Array.isArray(data)))
                error = new ErrorMessage(new lib_types.Types.Error(400, "invalidSyntax", `Expected 'data' to be a single complex value ${errorSuffix}`));
            // Make sure any bulkIds referenced in data can eventually be resolved
            else if (!waitingOn.every((id) => bulkIds.has(id)))
                error = new ErrorMessage(new lib_types.Types.Error(400, "invalidValue", `No POST operation found matching bulkId '${waitingOn.find((id) => !bulkIds.has(id))}'`));
            // If things look OK, attempt to apply the operation
                ...
                for (let referenceId of waitingOn) {
                    ...
                    try {
                        ...
                        else throw new lib_types.Types.Error(412, null, `Referenced POST operation with bulkId '${referenceId}' was not successful`);
                    }
                }
                ...
                if (!(ex instanceof lib_types.Types.Error)) 
                    ex = new lib_types.Types.Error(...(ex instanceof TypeError ? [400, "invalidValue"] : [500, null]), ex.message);
...
class SearchRequest {
    ...
    constructor(request) {
        ...
        if (request !== undefined && (!Array.isArray(schemas) || ((schemas.length === 1 && !schemas.includes(SearchRequest.#id)) || schemas.length > 1)))
            throw new lib_types.Types.Error(400, "invalidSyntax", `SearchRequest request body messages must exclusively specify schema as '${SearchRequest.#id}'`);
        ...
        try {
            ...
            throw new lib_types.Types.Error(400, "invalidValue", ex.message.replace(" in 'prepare' method of SearchRequest", ""));
        }
    }
    ...
    async apply(resourceTypes = Object.values(lib_resources.Resources.declared()), ctx) {
        // Make sure all specified resource types extend the Resource type class so operations can be processed correctly 
        if (!Array.isArray(resourceTypes) || !resourceTypes.every(r => r.prototype instanceof lib_types.Types.Resource))
